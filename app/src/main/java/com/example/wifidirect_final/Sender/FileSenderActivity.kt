package com.example.wifidirect_final.Sender
...
import com.example.wifidirect_final.BaseActivity
import com.example.wifidirect_final.DeviceAdapter
import com.example.wifidirect_final.DirectActionListener
import com.example.wifidirect_final.DirectBroadcastReceiver
import com.example.wifidirect_final.OnItemClickListener
import com.example.wifidirect_final.R
import com.example.wifidirect_final.Models.ViewState
import com.example.wifidirect_final.WifiP2pUtils
...
class FileSenderActivity : BaseActivity() {
    ...
    private fun initView() {
        supportActionBar?.title = "Отправитель файла"
        ...
        btnDirectDiscover.setOnClickListener {
            if (!wifiP2pEnabled) {
                showToast("Сначала нужно включить Wi-Fi")
                return@setOnClickListener
            }
            showLoadingDialog(message = "Поиск близлежащих устройств")
            wifiP2pDeviceList.clear()
            deviceAdapter.notifyDataSetChanged()
    ...
    private fun connect(wifiP2pDevice: WifiP2pDevice) {
        ...
        showLoadingDialog(message = "подключение deviceName: " + wifiP2pDevice.deviceName)
        showToast("подключение deviceName: " + wifiP2pDevice.deviceName)
        wifiP2pManager.connect(wifiP2pChannel, wifiP2pConfig,
            object : WifiP2pManager.ActionListener {
                ...
                override fun onFailure(reason: Int) {
                    showToast("Сбой подключения $reason")
                    dismissLoadingDialog()
                }
            })
    }
